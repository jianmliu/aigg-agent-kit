...
async function main() {
  ...
  // 赌注本金 = 银两(经济分离后),故 seed 走 startSilver。
  const mk = (name: string, silver: number) => world.createNpc({ name, owner: 'host:pal', background: '赌客', room: '苏州赌场', startSilver: silver });
  ...
  assert.ok(close(b.balanceSilver, 20), 'stake escrowed out of the bettor');
  ...
  const before = (await world.balanceSilver(haoke)) + (await world.balanceSilver(dugui));
  ...
  const haokeAfter = await world.balanceSilver(haoke);
  const duguiAfter = await world.balanceSilver(dugui);
  ...
  const preRefund = await world.balanceSilver(haoke);
  rr = await world.resolveRiceBet('refund');
  assert.equal(rr.ok, true);
  assert.equal(rr.outcome, 'NO');
  assert.ok(close(await world.balanceSilver(haoke), preRefund + 5), 'no winners → full refund');
