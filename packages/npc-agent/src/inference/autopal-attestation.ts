...
/** The metered fields a response signature binds (must match the Go signer). */
export interface ResponseDigestFields {
  requestHash: Hex;   // 0x…32 bytes: keccak256 of the request payload
  responseHash: Hex;  // 0x…32 bytes: keccak256 of the response body
  model: string;
  inputTokens: number;
  outputTokens: number;
}
...
/**
 * computeResponseDigest — keccak256(reqHash ‖ respHash ‖ model ‖ inTok ‖ outTok),
 * token counts big-endian uint64. Byte-identical to the gateway's
 * attest.PayloadDigest so both ends agree without a shared schema.
 */
export function computeResponseDigest(f: ResponseDigestFields): Hex {
  const packed = concat([
    hexToBytes(f.requestHash),
    hexToBytes(f.responseHash),
    stringToBytes(f.model),
    numberToBytes(BigInt(f.inputTokens), { size: 8 }),
    numberToBytes(BigInt(f.outputTokens), { size: 8 }),
  ]);
