...
/** → 中文 prompt 行,如「你现在很饿,也好几天没下酒馆了」;全足时返回 ''(talk 据此跳过注入)。
 *  轻量映射:对每条紧迫轴产「<程度><轴匮乏短语>」,内联中文(同 metabolism label 范式)。
 *  程度按值分档(<10 '已经'、<20 '很'、else '有些');多轴用「,」连接,末尾不加句号。 */
export function summarizeNeeds(s: NeedsState, axes?: Record<string, NeedsAxis>, thr = 30): string {
  const lacking = urgent(s, axes, thr);
  if (!lacking.length) return '';
  const parts = lacking.map((axis) => {
    const v = s[axis] ?? 100;
    const degree = v < 10 ? '已经' : v < 20 ? '很' : '有些';
    const phrase = LACK_PHRASE[axis] ?? `${axis}有些匮乏`;
    return `${degree}${phrase}`;
  });
  return parts.join(',');
}
