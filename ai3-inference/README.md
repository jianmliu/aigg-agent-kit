...
| `conformance/` — hermetic harness + grading CLI | **T7 done = V1** — local chain → legacy-tx deploy → fake-dsn → mock-dstack quote → stub-gateway (headers + SSE/trailers) → full matrix green incl. tamper cases + dcap column; `ai3-conformance --hermetic` or `--rpc/--registry/--dsn/--endpoint` for live pairs; **T9 done** — Phase-B `voucher-settlement` group: deposit → transferTo → 402 gate (voucher middleware on the stub gateway) → paid calls batch-settle on-chain → replay/expired rejected at the gate, fee>maxFee / nonce replay / expiry rejected by the contract, refund-window semantics (`--ledger/--ledger-endpoint/--ledger-provider/--user-key` for live pairs) |
